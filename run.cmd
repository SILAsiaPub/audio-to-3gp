...
set audio_param=-ac 1 -ar %audio_sample_rate% -b:a %target_bitrate% -strict unofficial -acodec %audio_encoder% 
...
:checks
if not exist "%inpath%\%infile_ext%" echo Input %infile_ext% files not found! Check your inpath settings in project.txt. & pause & goto :eof
if not exist "%ffmpeg%" echo Ffmpeg.exe not found! Check where ffmpeg.exe is installed and correct the path in your project.txt. & pause & goto :eof
